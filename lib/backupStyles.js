        ...
        async.series([
            callback => {
                ...
                if (fs.existsSync(filePath)) {
                    const fileModified = new Date(fs.statSync(filePath).mtime)
        ...
        async.series([
            callback => {
                ...
                if (fs.existsSync(styleFilePath) && fs.existsSync(filePath)) {
                    const fileModified = new Date(fs.statSync(styleFilePath).mtime)
                    const timeDifference = styleModified.getTime() - fileModified.getTime();
...
function listStyles(stylesService, cb) {
    ...
    const styles = [];
    stylesService.listStyles().eachPage((err, res, next) => {
